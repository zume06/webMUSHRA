    ...
    for (i = 0; i < trials.length; ++i) {
      ...
      if (trial.type === "mushra") {
        ...
        for (j = 0; j < trial.responses.length; ++j) {
          trPC = document.createElement("tr");
          tdPCReference = document.createElement("td");
          tdPCSampleA = document.createElement("td");
          tdPCSampleB = document.createElement("td");

          tdPCres1 = document.createElement("td");
          tdPCres2 = document.createElement("td");
          tdPCres3 = document.createElement("td");
          tdPCres4 = document.createElement("td");

          var response = trial.responses[j];

          $(tdPCres1).append(response.answerTimbre);
          $(tdPCres2).append(response.answerRhythm);
          $(tdPCres3).append(response.answerMelody);
          $(tdPCres4).append(response.answerTotal);
          //$(tdPCReference).append(response.Reference);
          //$(tdPCSampleA).append(response.SampleA);
          //$(tdPCSampleB).append(response.SampleB);
          $(trPC).append(tdPCres1);
          $(trPC).append(tdPCres2);
          $(trPC).append(tdPCres3);
          $(trPC).append(tdPCres4);
          //$(trPC).append(tdPCReference);
          //$(trPC).append(tdPCSampleA);
          //$(trPC).append(tdPCSampleB);
          $(table).append(trPC);
        }
        console.log("table:", table)
        trEmpty = $("<tr height='8px'></tr>");
      ...
      else if (trial.type === "enquete") {
        trEnq = document.createElement("tr");
        $(trEnq).append(trial.responses);
      }
    }

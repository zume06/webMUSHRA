...
GetSetNum.prototype.render = function (_parent) {
  ...
  $.get('./configs/2024sp/setnums/' + self.testID + '/setnums.csv', function (data2) {
    $.get('./results/' + self.testID + '/finish_no.csv', function (data3) {
      const setnumArray = data2.split('\n');
      self.setnumArray = setnumArray;
      const finishnumArray = data3.split('\n');
      self.finishnumArray = finishnumArray;
      self.availnumArray = self.setnumArray.filter(item => !finishnumArray.includes(item));
      $.get('./results/' + self.testID + '/counts.csv', function (data1) {
        ...
        button.click(function () {
          count++;
          //console.log("self.availnumArray", self.availnumArray);
          if (self.availnumArray.length == 0) {
            setnum = Math.floor(Math.random() * 59);
          } else {
            var remain = count % self.availnumArray.length;
            setnum = self.availnumArray[remain];
          }

  ...
  $.get('./configs/2024sp/setnums/' + self.testID + '/setnums.csv', function (data2) {
    ...
    $.get('./results/' + self.testID + '/counts.csv', function (data1) {
      ...
      button.click(function () {
        ...
        var url = "http://localhost:8000/?config=2024sp/configfiles/config_set" + setnum + ".yaml";
      ...
      button.click(function () {
        count++;
        setnum = self.setnumArray[count];
        countDisplay.text("実験URL: http://localhost:8000/?config=2024sp/configfiles/config_set" + setnum + ".yaml");
        self.count = count;
        userID = $("#inputBox").val();
        self.userID = userID;
        self.save();

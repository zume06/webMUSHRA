...
EnquetePage.prototype.render = function (_parent) {
  ...
  radioChoice.find("input[type='radio']").bind("change", (function () {
    if ($('#radio-choice-yes').prop('checked') || $('#radio-choice-no').prop('checked')) {
      console.log("yes checked:", $('#radio-choice-yes').prop('checked'));
      console.log("no checked:", $('#radio-choice-no').prop('checked'));
...
EnquetePage.prototype.save = function () {
  var ans = $("#radio-choice input[name='radio-choice']:checked").val();
  this.choice = ans
};
...
EnquetePage.prototype.store = function () {
  ...
  trial.responses[trial.responses.length] = this.choice;
